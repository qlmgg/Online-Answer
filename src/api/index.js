...
// 获取考卷
export const getPapers = ({ page = 1, limit = 8, keywords = "", state = "", selfid = "" }) =>
    request.get(`/papers?page=${page}&limit=${limit}&keywords=${keywords}&state=${state}&selfid=${selfid}`);
...
export const getQuestions = ({
    ...
    request.get(
        `/questions?keywords=${keywords}&ban=${ban}&public=${_public}&page=${page}&limit=${limit}&type=${type}&exampaper=${exampaper}&selfid=${selfid}`
    );
...
// 获取答卷记录
export const getAnswers = ({ exampaper = "", page = 1, limit = 10, state = "", score = '' }) =>
    request.get(`/answers?exampaper=${exampaper}&page=${page}&limit=${limit}&state=${state}&score=${score}`);
...
// 更新答卷
export const updateAnswer = (id = "", data = {}) => request.put(`/answers/${id}`, data);

// 获取所有用户
export const getUsers = ({ page = 1, keywords = "", role = 0, ban = false, gender = "" }) =>
    request.get(`/users?page=${page}&keywords=${keywords}&role=${role}&ban=${ban}&gender=${gender}`);
...
// 搜索
export const search = ({ keywords = "", type = 0, limit = 10, page = 1 } = {}) =>
    request.get(`/hotwords/search?keywords=${keywords}&type=${type}&page=${page}&limit=${limit}`);

// 排行榜
export const getRankingList = ({ rankDate = "today", rankType = "paper" } = {}) =>
    request.get(`/rankinglist?rankType=${rankType}&rankDate=${rankDate}`);
